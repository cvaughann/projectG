...
for i in range(len(chromosome)):
    if chromosome[i] == key["Chromosome"]:
        # print("Chromosomes match")
        if strand[i] == key["Strand"]:
            # print("Strands match")
            # Next I will check if there is any overlap between the start and stop indices
            if start[i] >= key["Start"] and stop[i] <= key["Stop"]:
                # print("Indices match")                
                protein_counts[protein[i]] += 1
            elif start[i] <= key["Start"] and stop[i] >= key["Start"]:
                print("Indices match")
                protein_counts[protein[i]] += 1
            elif start[i] <= key["Stop"] and stop[i] >= key["Stop"]:
                print("Indices match")
                protein_counts[protein[i]] += 1
            elif start[i] >= key["Stop"]:
                break
end_time = time.time()
print(protein_counts)
print("Ran through all", len(chromosome), "lines in", end_time-start_time, "seconds")
